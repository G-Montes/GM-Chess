...
public class Pawn extends Piece {
    ...
    public Collection<Move> calculateLegalMoves(final Board board) {
        ...
        for(int currentCandidateOffset : CANDIDATE_MOVE_VECTOR){
            ...
            /*For this, there's a check for edge cases that would result from having an offset to 9. For further
             *clarification, see *A
             */
...
/*A The way this works is that each column has it's own method. If the pawn is NOT in the column then the test will
 *fail, meaning that the piece is not in the correct column to trigger an exclusion. In case the pawn IS in the
 *appropriate column, then the test will only return true if the candidate offset would trigger an exclusion given the
 *pawn color. For example, if the candidateOffset is 7, then a black pawn on the first column would result in true in
 *the isFirstColumnExclusion but false in the isEighthColumnExclusion (since the black is on the first column). A white
 *piece in the same conditions on the other hand would return false for both methods.
 *Refer to Logic_Behind_Pawn_Attack_Implementation for all possible cases. */
